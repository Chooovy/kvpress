...
class CacheIndexer(nn.Module):
    def __init__(self, config, indexer_args):
        ...
        # only cache k use recent buffer_len queries to score full keys
        self.register_buffer("k_cache", torch.empty(0, 0, self.head_dim), persistent=False)
        self.register_buffer("q_cache", torch.empty(0, 0, self.n_heads, self.head_dim), persistent=False)
        self.register_buffer("weights_cache", torch.empty(0, 0, self.n_heads), persistent=False)


    def forward(self, x, freqs_cis, mask, use_cache=False):
        bsz, seqlen, _ = x.size()
        ...
        if use_cache:
            if self.k_cache.shape[1] == 0:
                self.k_cache, self.q_cache, self.weights_cache = k, q, weights
            else:
                self.k_cache = torch.cat([self.k_cache, k], dim=1)
                self.q_cache = torch.cat([self.q_cache, q], dim=1)
                self.weights_cache = torch.cat([self.weights_cache, weights], dim=1)
            full_k, full_q, full_weights = self.k_cache, self.q_cache, self.weights_cache
        else:
            full_k, full_q, full_weights = k, q, weights
    ...
    def forward_cache(self, x, freqs_cis, mask):
        bsz, seqlen, _ = x.size()
        if x.dtype != self.w_q.weight.dtype:
            x = x.to(self.w_q.weight.dtype)
        ...
        return self.k_cache, self.q_cache, self.weights_cache

    def get_cache_score(self):
        logits = torch.einsum("bshd,btd->bsht", self.q_cache, self.k_cache)
        logits = _apply_activation(logits, self.activation) * self.weights_cache.unsqueeze(-1)
        indexer_scores = logits.sum(dim=2)  # (bsz, cache_len, cache_len)
    ...
    def compress_cache_by_indices(self, token_indices):
        ...
        self.q_cache = self.q_cache.gather(1, token_indices.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, self.q_cache.shape[2], self.q_cache.shape[3])).contiguous()
        self.weights_cache = self.weights_cache.gather(1, token_indices.unsqueeze(-1).expand(-1, -1, self.weights_cache.shape[-1])).contiguous()

    def reset_cache(self):
        self.k_cache = torch.empty(0, 0, self.head_dim)
        self.q_cache = torch.empty(0, 0, self.n_heads, self.head_dim)
        self.weights_cache = torch.empty(0, 0, self.n_heads)

@dataclass
class CacheIndexerScorePress(ScorerPress):
    ...
    def post_init_from_model(self, model, force_reinit=False):
        ...
        for layer in language_model.layers:
            ...
            indexer = CacheIndexer(model.config, args).to(model.device, dtype=model.dtype)
    ...
    def indexer_logits(self, module, hidden_states, kwargs):
        ...
        return getattr(module, self.scorer_attr)(hidden_states, freqs, mask, use_cache=False)
    ...
    def score(self, module, hidden_states, keys, values, attentions, kwargs):
        indexer: CacheIndexer = getattr(module, self.scorer_attr)
        # q_len = hidden_states.size(1)
        k_len = keys.size(2)

        is_decoding = kwargs.get("is_decoding", False)
        
        # 在decode阶段，hidden_states可能为None，使用indexer cache的长度
        if is_decoding:
        ...
        if is_decoding:
            indexer_scores = indexer.get_cache_score()
            token_scores = indexer_scores.mean(dim=1)
        else:
            indexer_scores = indexer(hidden_states, freqs, mask, use_cache=False)
            if q_len == 1:
...
def load_model_with_indexer_press_cache(model_path, model_kwargs=None):
    ...
    if os.path.exists(checkpoint_path):
        ...
        if has_trained_scorer:
            dummy_press = CacheIndexerScorePress(compression_ratio=0.0)
            dummy_press.post_init_from_model(model, force_reinit=False)
            
            model.load_state_dict(state_dict, strict=False)
            print("✓ Loaded trained CacheIndexerScorePress weights from checkpoint")
