...
class PrefixIndexerConfig(ScalarIndexerConfig):
    ...
    ``pos_slope``. Reporting ``0`` is also what lets the press's RoPE plumbing and
    :mod:`~.cross_replay` drive this scorer through the scalar arm's code path unchanged.
    """
...
class PrefixIndexer(ScalarIndexer):
    ...
    def __init__(self, config: PrefixIndexerConfig):
        ...
        # Bias-free, like the other two scorers. q/k/v all read the *same* in_norm(h) that the
        # readout uses, so the wide fp32 norm is paid once.
    ...
    def score_keys(
        ...
        *,
        ...
        :mod:`~.cross_replay` and the press call it unchanged.
...
def score_variance_profile(
    ...
    for lo, hi in zip(edges[:-1].tolist(), edges[1:].tolist()):
        ...
        if n < 2:
            ...
            continue
        # Detrend within the bin, weighted by validity: pos_slope contributes a deterministic
        # ramp whose variance would otherwise read as content spread. The weighting is
        # load-bearing -- fitting the slope over masked entries too lets a MASK_NEG sentinel
        # set the trend and the residual then dwarfs the real signal.
