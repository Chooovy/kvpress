...
# copy .bashrc to /home/devuser/.bashrc
cp /root/.bashrc /home/devuser/
# always switch to devuser for devmachine -> hinders the actions that need root (e.g., evaluation)
# echo "su devuser" >> /root/.bashrc
# Create a new detached tmux session named 'hold' and run the matrix multiplication program
tmux new -d -s hold
tmux send-keys -t hold "python default_job.py" ENTER

# setup vscode tunnel if VSCODE_CLI_ACCESS_TOKEN is set
# if [ -n "$VSCODE_CLI_ACCESS_TOKEN" ]; then
#     tmux new -d -s codeserver
#     tmux send-keys -t codeserver "code tunnel user login --provider github --access-token=$VSCODE_CLI_ACCESS_TOKEN && code tunnel --accept-server-license-terms" ENTER
# fi


# keep container running
tail -f default_job.py
