...
def test_rope_is_rejected_and_rope_dim_is_zero():
    ...
    ``rope_dim == 0`` is what routes the press's RoPE plumbing down the scalar arm's path
    unchanged, so it is part of the interface rather than an implementation detail.
    """
    p = _prefix()
