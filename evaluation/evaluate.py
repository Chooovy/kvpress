...
class EvaluationRunner:
    ...
    def _load_and_prepare_dataset(self):
        ...
        if data_dir and dataset_name not in ["math500", "aime25", "longbench"]:
            load_kwargs["data_dir"] = data_dir
        if dataset_name == "zero_scrolls":
            cfgs = get_dataset_config_names(path)   # 10 个任务 config :contentReference[oaicite:2]{index=2}
            dfs = []
            for cfg in cfgs:
                tmp = load_dataset(path, cfg, split="test", trust_remote_code=True).to_pandas()  # 需要 trust_remote_code :contentReference[oaicite:3]{index=3}
                if "input" in tmp.columns:
                    tmp = tmp.drop_duplicates(subset=["input"])
                tmp["task"] = cfg
                dfs.append(tmp)
