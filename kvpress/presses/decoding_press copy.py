...
from kvpress.presses.indexer_score_press_cache import IndexerScorePressCache
...
class DecodingPress(BasePress):
    ...
    def forward_hook(self, module: nn.Module, input: list[torch.Tensor], kwargs: dict, output: list):
        ...
        if kwargs["cache_position"][-1] <= q_len:
            ...
            if isinstance(self.base_press, IndexerScorePressCache):
                indexer = getattr(module, self.base_press.scorer_attr, None)
                if indexer is not None:
                    _ = indexer(hidden_states, freqs_cis=None, mask=None, use_cache=True, accumulate=True)
            return output
        ...
        if (self.layer_step_counts[layer_idx] >= self.compression_interval) or (q_len >= self.target_size):
            ...
            if isinstance(self.base_press, IndexerScorePressCache):
                ...
                if indexer is not None:
                    with torch.no_grad():
                        _ = indexer(buffered_hidden_states, freqs_cis=None, mask=None, use_cache=True, accumulate=True)
                
                keys, values, kept_indices = self.compress_with_indices(module, buffered_hidden_states, keys, values, attentions, kwargs)
                
                if indexer is not None and kept_indices is not None:
                    # kept_indices: (bsz, n_heads, n_kept) or (bsz, n_kept)
                    bsz = kept_indices.shape[0]
                    if kept_indices.dim() == 3:
                        kept_indices = kept_indices[:, 0, :]  # (bsz, n_kept)
                    
                    indexer.k_cache = indexer.k_cache.gather(1, kept_indices.unsqueeze(-1).expand(-1, -1, indexer.k_cache.shape[-1]))
                    indexer.q_cache = indexer.q_cache.gather(1, kept_indices.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, *indexer.q_cache.shape[2:]))
                    indexer.weights_cache = indexer.weights_cache.gather(1, kept_indices.unsqueeze(-1).expand(-1, -1, indexer.weights_cache.shape[-1]))
            else:
    ...
    def compress_with_indices(self, module, hidden_states, keys, values, attentions, kwargs):
        k_len = keys.shape[2]
        target_compression_ratio = self._find_target_compression_ratio(k_len, self.target_size)
