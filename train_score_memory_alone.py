...
def parse_args() -> argparse.Namespace:
    ...
    # RULER loader options (used by data_load.load_ruler)
    parser.add_argument(
        "--ruler_supervise_prefix",
        action="store_true",
        help="Supervise generating answer_prefix+answer for RULER (more labeled tokens, stronger signal).",
    )
    ...
    parser.add_argument("--memory_loss_weight", type=float, default=1.0, help="Weight for memory residual MSE.")
...
def main():
    ...
    train_ruler, eval_ruler = load_ruler(tokenizer, args)
    datasets_train.append(train_ruler)
    datasets_eval.append(eval_ruler)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            if global_step % args.logging_steps == 0:
                ...
                accelerator.print(
                    f"Epoch {epoch} Step {global_step} / Total Steps {total_steps}: "
                    f"train_loss={avg_loss:.6f}, train_grad_norm={avg_grad_norm:.6f}"
                )
                running_loss = 0.0
                running_grad_norm = 0.0
