dataset=needle_in_haystack
data_dir=("")
model=(
    "/aifs4su/guhao/checkpoints/llama3-8b-query_indexer-max"
    )
compression_ratios=(0.5 0.75)
press_names=(
    "query_indexer_kvzip_max"
)
context_lengths=(1000 1500 2000 2500 3000 3500 4000 4500 5000 5500 6000 6500 7000 7500 8000 8500 9000 9500 10000 10500 11000 11500 12000 12500)
needle_depths=(15 25 35 45 55 65 75 85 95)
device="cuda"
gpu_ids=(0 1 2 3 4 5 6 7)
max_jobs=${#gpu_ids[@]}
next_gpu_idx=0

for press_name in "${press_names[@]}"; do
    for model in "${model[@]}"; do
        for data_dir in "${data_dir[@]}"; do
            for compression_ratio in "${compression_ratios[@]}"; do
                for max_context_length in "${context_lengths[@]}"; do
                    for needle_depth in "${needle_depths[@]}"; do
                        # Wait for a free GPU slot if all are busy
                        while [ "$(jobs -rp | wc -l)" -ge "$max_jobs" ]; do
                            wait -n
                        done

                        device_id=${gpu_ids[$next_gpu_idx]}
                        next_gpu_idx=$(( (next_gpu_idx + 1) % max_jobs ))

                        echo "press=$press_name cr=$compression_ratio ctx=$max_context_length depth=$needle_depth gpu=$device_id"
                        CUDA_VISIBLE_DEVICES=$device_id python evaluate_bak_xintong.py \
                            --dataset $dataset \
                            --model "$model" \
                            --data_dir "$data_dir" \
                            --press_name $press_name \
                            --compression_ratio $compression_ratio \
                            --max_context_length "$max_context_length" \
                            --needle_depth "$needle_depth" \
                            --device ${device}:0 \
                            --compress_questions False \
                            --output_dir "./results_needle_guhao" &
                    done
                done
            done
        done
    done
done


wait
echo "All evaluation tasks completed."