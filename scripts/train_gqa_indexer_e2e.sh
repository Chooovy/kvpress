...
OUT="${OUT:-/apdcephfs_zw31/share_303843174/user/marcushaogu/models/Qwen-3-8B-gqa_indexer_e2e}"
...
case "$MODE" in
  smoke)
    # Single-process on purpose: one real traceback beats eight interleaved ones when what is
    # being checked is whether the path runs at all.
  ...
  stage1_16k)
    ...
    # sequence, so tokens/step equals the sequence length. That is 8x fewer tokens/step than the
    # 8K DDP run -- the price of the longer context. FFN_SP=4 would give 2 replicas at ~2x the
    # per-rank memory; use it if 16K fits at that setting.
    exec "${LAUNCH[@]}" -m scripts.train_gqa_indexer_e2e \
      --data-root "$DATA_ROOT" --model "$MODEL" $(data_args) \
      --schedule "${SCHEDULE:-8192:300,16384:300,32768:900}" \
      ...
      --num-workers "${WORKERS:-2}" \
      --out "$OUT/stage1_16k" --metrics-file "$OUT/stage1_16k/metrics.jsonl" \
      --save-every "${SAVE_EVERY:-200}" --log-every "${LOG_EVERY:-10}"
    ;;
